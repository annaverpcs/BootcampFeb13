# Author: Oleksandr Dubynskyi
  ...
  And I type "AlexD Unique appointment theme 1" into element with xpath "//textarea[@id='note']"
      Then I click on element with xpath "//select[@name='employee_id']"
      And I wait for 1 sec
      Then I click on element with xpath "//select[@name='employee_id']/..//option[4]"
      And I type "03/19/2024" into element with xpath "//input[@id='date']"
      And I wait for 1 sec
      And I click on element with xpath "//button[contains(text(),'08:30 AM')]"
   ...
   #verify that the appointment is created and is in the List of appointments
      Then I wait for element with xpath "//p[contains(text(),'AlexD Unique appointment theme 1')]" to be present
  Then element with xpath "//p[contains(text(),'AlexD Unique appointment theme 1')]" should contain text "AlexD Unique appointment theme 1"
      Then I wait for element with xpath "//p[contains(text(),'AlexD Unique appointment theme 1')]//ancestor::article/div[2]/span[contains(text(),'update')]" to be present
And I click on element with xpath "//p[contains(text(),'AlexD Unique appointment theme 1')]//ancestor::article/div[2]/span[contains(text(),'update')]"
...
And element with xpath "//h2[contains(text(),'Are you sure you want to cancel appointment?')]" should contain text "Are you sure you want to cancel appointment?"
      ...
      And element with xpath "//p[contains(text(),'AlexD Unique appointment theme 1')]" should not be present